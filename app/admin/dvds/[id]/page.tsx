...
function resolveTrackRecord(entry: unknown): AnyRecord | null {
  if (!entry || typeof entry !== 'object') {
    return null;
  }
  const record = entry as AnyRecord;
    ...
    .map((entry) => {
      ...
      const label =
        typeof (record as { label?: unknown }).label === 'string'
          ...
          : typeof (record as { publishing_company?: unknown }).publishing_company === 'string'
            ? ((record as { publishing_company?: string }).publishing_company ?? '')
            : '';
...
export default function EditDvdPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    toast.success('DVD atualizado com sucesso');
    const refreshed = await response.json().catch(() => null);
    if (refreshed && typeof refreshed === 'object') {
      const coverImage = toUploadedImage((refreshed as { cover?: unknown }).cover);
      setCover(coverImage ? [coverImage] : []);
      const trackEntries = Array.isArray((refreshed as { track?: unknown[] }).track)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Atualize os dados do DVD e gerencie as faixas.</p>
      </div>
      <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <Card>
          ...
          <CardFooter className="justify-between">
            <Button type="button" variant="ghost" onClick={() => router.push('/admin/dvds')}>
              Voltar
            </Button>
