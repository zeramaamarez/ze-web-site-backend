...
const formSchema = textSchema.extend({
...
export default function NewTextPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (!response.ok) {
      ...
      toast.error((data as { error?: string }).error || 'Erro ao criar texto');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Novo texto</h1>
        <p className="text-sm text-muted-foreground">Cadastre um novo texto ou artigo.</p>
      </div>
      <form className="grid gap-6 md:grid-cols-2" onSubmit={form.handleSubmit(onSubmit)}>
        <div className="space-y-4">
          ...
          <Button type="submit" disabled={form.formState.isSubmitting}>
            {form.formState.isSubmitting ? 'Salvando...' : 'Salvar texto'}
          </Button>
        </div>
        <div className="space-y-4">
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label>Capa</Label>
            <ImageUpload value={cover} onChange={setCover} folder="texts" />
          </div>
        </div>
      </form>
    </div>
