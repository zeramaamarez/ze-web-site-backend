...
async function getStats() {
  ...
  const [booksTotal, booksPublished, cdsTotal, cdsPublished, dvdsTotal, dvdsPublished, clipsTotal, clipsPublished, cdTracks, dvdTracks] =
    await Promise.all([
  ...
  const uploadsSize = await UploadFileModel.aggregate([
    {
      $group: {
        _id: null,
        total: { $sum: '$size' }
  ...
  return {
    ...
    latest: latest.flat().sort((a, b) => (b.updatedAt?.getTime() || 0) - (a.updatedAt?.getTime() || 0)).slice(0, 5),
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{(stats.uploadsSize / 1024).toFixed(2)} MB utilizados</CardDescription>
          </CardHeader>
        </Card>
      </div>
      <Card>
            ...
            {stats.latest.map((item, index) => (
              <li key={index} className="flex items-center justify-between text-sm">
                ...
                <span className="text-muted-foreground">
                  {item.updatedAt ? format(item.updatedAt, "dd 'de' MMMM 'de' yyyy", { locale: ptBR }) : 'N/A'}
                </span>
              </li>
            ))}
          </ul>
