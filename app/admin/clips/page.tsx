...
const columnOptions: ColumnOption[] = [
  { key: 'cover', label: 'Thumbnail', defaultVisible: false },
  { key: 'title', label: 'Título', defaultVisible: true },
  { key: 'url', label: 'Link YouTube', defaultVisible: true },
...
export default function ClipsPage() {
  ...
  const columnPreferences = useColumnPreferences('table-columns-clips', columnOptions);
  ...
  const fetchClips = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.error(data?.error || 'Erro ao carregar clips');
        return;
      }
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          const data = (await response.json().catch(() => null)) as { error?: string } | null;
          toast.error(data?.error || 'Erro ao remover clip');
          return;
        }
        toast.success('Clip removido');
        if (clips.length <= 1 && page > 1) {
  ...
  const allColumns = useMemo<EnhancedColumn<ClipItem>[]>(
    () => [
      {
        key: 'cover',
        label: 'Thumbnail',
      ...
      },
      {
        ...
        sortable: true,
        defaultVisible: false,
        render: (item) => extractYear(item.createdAt) || '—'
      },
      {
        key: 'actions',
        label: 'Ações',
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
          {item.cover?.[0]?.url ? (
            ...
            <div className="flex h-32 w-full items-center justify-center rounded-xl border border-dashed bg-muted/50 sm:h-20 sm:w-32">
              <ImageIcon className="h-6 w-6 text-muted-foreground" />
            </div>
          )}
          <div className="flex-1 space-y-2">
            <div>
              ...
              >
                <Youtube className="h-4 w-4" /> Abrir vídeo
              </a>
            </div>
            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
              {item.createdAt && <span className="rounded-full bg-muted px-2 py-1">Criado em {extractYear(item.createdAt)}</span>}
              <Badge className={item.published_at ? 'bg-green-100 text-green-700 hover:bg-green-100' : 'bg-blue-100 text-blue-700 hover:bg-blue-100'}>
                {item.published_at ? 'Published' : 'Draft'}
              </Badge>
            </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gerencie os videoclipes com thumbnails e filtros avançados.</p>
        </div>
      ...
      <DataTable
        ...
        emptyMessage={isLoading ? 'Carregando clips...' : 'Nenhum clip encontrado.'}
        isLoading={isLoading}
        pageSize={pageSize}
